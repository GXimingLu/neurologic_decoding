...
from s2s_lm.s2s_ft.constrain_decoding import UnilmConstrainDecode
from s2s_lm.s2s_ft.modeling_decoding import BertConfig
from transformers.tokenization_bert import whitespace_tokenize
import s2s_lm.s2s_ft.s2s_loader as seq2seq_loader
from s2s_lm.s2s_ft.utils import load_and_cache_examples
from transformers import BertTokenizer, RobertaTokenizer
from s2s_lm.s2s_ft.tokenization_unilm import UnilmTokenizer
...
def main():
    ...
    mask_word_id, eos_word_ids, sos_word_id, period_id = tokenizer.convert_tokens_to_ids(
    ...
    for model_recover_path in [args.model_path.strip()]:
        ...
        model = UnilmConstrainDecode.from_pretrained(
        ...
        with open(args.constraint_file, 'r') as f:
            constraints_list = []
            for line in f:
                constraints_list.append(json.loads(line))
        if args.subset > 0:
            logger.info("Decoding subset: %d", args.subset)
            input_lines = input_lines[:args.subset]
            constraints_list = constraints_list[:args.subset]
        ...
        with tqdm(total=total_batch) as pbar:
            ...
            while next_i < len(input_lines):
                ...
                with torch.no_grad():
                    ...
                    traces = model.decode(input_ids, token_type_ids,
                                   position_ids, input_mask, task_idx=task_idx, mask_qkv=mask_qkv, constraints=constraints)
                    if args.beam_size > 1:
